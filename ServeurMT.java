...
public class ServeurMT extends Thread {
	...
	public synchronized void run() {
		...
		try {
			...
			while(true) {
				...
				nbJoueurs++;				//à chaque joueur en plus, le nombre des joueurs connectés augmentes 
				joueursConnectes.add(s2);
				nbJoueurs++;
				String IP1 = s1.getRemoteSocketAddress().toString();
				System.out.println("Connexion du client numéro 1 réçue. IP= "+IP1);
				String IP2 = s2.getRemoteSocketAddress().toString();
				System.out.println("Connexion du client numéro 2 réçue. IP= "+IP2);
	...
	class Conversation extends Thread{
		...
		public synchronized void run() {
			...
			//On utilise ce thread pour l'échange des phrases entre les duex joueurs
				...
				int i =0;
		          ...
		          public synchronized void run() {
		        	  try {
		            	 while(true){
		            		 synchronized (this){
		            	 pred1.println("Veuillez écrire votre phrase:"); //On demande au joueur1 d'écrire une phrase de 50 caractères.
		                 req1 = plec1.readLine();
		            	...
		            	//Dans cette boucle on renvoie la phrase du joueur1 sans synonymes et sans le 30% des caractères au joueur2 (on utilise les méthode isSyno et supprimer de la classe dictionnaire).
		            	//On demande au joueur2 d'écrire sa phrase,on vérifie s'il a deviné. S'il y a le mot but, on arète le jeux,autrement on continue jusqu'à la cinquième essai.
		            	 while(req1!=null){   
		                 ...
		                 synchronized (this){
			                   ...
			                   while(req2.length()>50){
			                	   pred2.println("Veuillez écrire une phrase de 50 caractère au maximum.");
			                	   req2 = plec2.readLine();
			                	   pred2.println("S'il vous plait attendez");
			                   }
		                   }
		                 if(req2!=null) {
		                	   ...
		                	   pred1.println(req2);
		                	   if(trouve){  //cas où le joueur1 a deviné:on ferme le jeux.
		                    ...
		                    i++;
		                    if(i==5){ //cas où les joueurs ont fait 5 essais:on ferme le jeux.
	...
	public static void main (String[] args) {
		//On a crée 10 mots,mais le dictionnaire peut etre modifié et élargi(?) en ajoutant des nouveaux Mot
